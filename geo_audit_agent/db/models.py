...
class ObservationEvidence(SQLModel, table=True):
    """Immutable provider observation used as the source for displayed metrics."""

    __tablename__ = "observation_evidence"
    __table_args__ = (
        Index("idx_observation_audit_provider", "audit_id", "provider"),
        {"extend_existing": True},
    )
    ...
    prompt_id: str = Field(max_length=100, index=True)
    prompt_version: str = Field(max_length=30)
    execution_mode: str = Field(max_length=20, index=True)
