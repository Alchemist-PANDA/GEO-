...
def _citation_answer(context: dict) -> str:
    brand = _brand(context)
    citation_rate = _pct(context.get("citation_rate"))

    if citation_rate is None:
        return (
            f"### 🔗 Citation Rate for {brand}\n\n"
            "Citation rate is unavailable until an evidence-backed audit has collected cited source URLs. "
            "No percentage has been estimated."
        )
...
def _sentiment_answer(context: dict) -> str:
    ...
    return (
        ...
        "Sentiment is unavailable because the current evidence does not contain a validated sentiment measurement. "
        "No percentage has been estimated."
    )
...
def _competitor_answer(context: dict, user_message: str) -> str:
    ...
    if summary.get("status") != "measured":
        return (
            f"Competitor comparison for **{brand}** is unavailable. Collect like-for-like live observations for "
            "the brand and every competitor before calculating a ranking. No values have been estimated."
        )
