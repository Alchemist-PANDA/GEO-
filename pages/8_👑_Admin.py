...
# ── Restrict access using the verified Supabase session ──
...
with get_session() as s:
    ...
    paid_invoices = s.exec(select(InvoiceRequest).where(InvoiceRequest.status == "paid")).all()
    paid_profiles = s.exec(select(UserProfile).where(UserProfile.tier != "free")).all()
    total_revenue = sum(invoice.amount for invoice in paid_invoices)
    monthly_recurring_revenue = sum(TIER_CONFIG.get(profile.tier, {}).get("price", 0) for profile in paid_profiles)

col1, col2, col3, col4 = st.columns(4)
col1.metric("Total Users", total_users)
col2.metric("Paid Users", paid_users)
col3.metric("Recorded paid invoices", f"${total_revenue:,.2f}")
col4.metric("MRR from active tiers", f"${monthly_recurring_revenue:,.2f}")
...
with tab4:
    ...
    st.info(f"📧 Signed in administrator: {admin.email}")
