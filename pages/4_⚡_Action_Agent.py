...
from geo_audit_agent.ui.theme import render_empty, render_page_header

st.set_page_config(page_title="Action Agent", page_icon="⚡", layout="wide")
user = require_user_or_demo()

render_page_header("⚡", "Action Agent", "Turn evidence-backed visibility gaps into a prioritized, approval-gated execution plan.")
...
if st.button("Execute approved actions", type="primary", use_container_width=True):
    ...
    for result in state.action_results:
        icon = {"deployed": "✅", "complete": "✅", "fallback": "📄", "awaiting_approval": "⏳"}.get(result["status"], "⚠️")
        with st.expander(f"{icon} {result['action_id']} — {result['status']}"):
